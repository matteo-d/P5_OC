...
.then((productResp) => {
  ...
  for(let i = 0; i < productResp.length; i++) {
   ...
   html += `
   <div class="product">
        <img src="${productResp[i].imageUrl}" alt="Ours ${productResp[i].name}" class="ourspic">
        <article class="product_infos">
              <h2 id="name_product_${productResp[i].name}">${productResp[i].name}</h2>
              <p class="product_description" id="description_product_${productResp[i].name}">${productResp[i].description}</p>   
              <p class="price" id="price_product_${productResp[i].name}">${(productResp[i].price/100).toFixed(2).replace(".",",")} €</p>
              <a href="./produits.html?${productResp[i]._id}">Voir</a>
      </div >
`
  }
